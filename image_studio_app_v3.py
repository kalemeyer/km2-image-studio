...
_REMBG_REQUIREMENT, _REMBG_ERROR = rembg_requirement()

if _REMBG_REQUIREMENT:
    _REMBG_INSTALL_HINT = f"pip install {_REMBG_REQUIREMENT}"
else:
    _REMBG_INSTALL_HINT = _REMBG_ERROR or "pip install rembg"
    ...
    if cfg.use_rembg:
        if not _HAVE_REMBG:
            raise RuntimeError(
                ...
                f"Install with: {_REMBG_INSTALL_HINT}"
            )
...
class AppBase:
    ...
    def _install_rembg(self):
        ...
        if not _REMBG_REQUIREMENT:
            reason = _REMBG_ERROR or "rembg is not available for this Python version."
            self._log(f"Cannot install rembg automatically: {reason}")
            messagebox.showerror("rembg unavailable", reason)
            return
        ...
        try:
            ...
            messagebox.showerror(
                "Installation failed",
                f"pip could not install {_REMBG_REQUIREMENT}: {exc}",
            )
            return
        except Exception as exc:  # noqa: BLE001
            self._log(f"rembg installation failed: {exc}")
    ...
    def _help(self):
        msg = ("Workflow:\n"
               ...
               + (
                   ""
                   if _HAVE_REMBG
                   else f"\nNOTE: rembg not detected. Install with: {_REMBG_INSTALL_HINT}"
               ))
    ...
    def _process_queue(self):
        ...
        with open(manifest_path, "w", newline="", encoding="utf-8") as csvfile:
            ...
            if cfg.use_rembg and not _HAVE_REMBG:
                self._log(
                    "⚠️ Background removal requested but 'rembg' not installed. "
                    f"Install with: {_REMBG_INSTALL_HINT}"
                )
